...
Existing solutions are mainly based on either x-ray or RFID. With x-ray, the patient needs to be scanned using a scanner on wheels. Metal object obviously will be visible, while other items such as swabs needs to have metal strips wowen to be detected and the surgery team has to wear lead aprons. Some items have passive RFID-circuits embedded and can be detected by a hand heldscanner. Both alternatives are both impractical and entail flaws.
